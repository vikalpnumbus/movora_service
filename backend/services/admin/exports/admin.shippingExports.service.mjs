...
class Service {
  ...
  async getData({ filters, exportJobId }) {
    try {
      ...
      for (let i = 1; i <= AdminShippingExportsHandler.MAX_PRODUCTS; i++) {
      ...
      const transformStream = new Transform({
        ...
        async transform(e, _, callback) {
          ...
          let row = {
            ...
            "Freight Charge": e.freight_charge,
            "COD Price": e.cod_price,
            Zone: e.zone,
      ...
      const uploadPath = path.join(dir, `${Date.now()}.shipping.csv`);
