...
class Service {
  constructor() {
    this.error = null;
    this.repository = FactoryRepository.getRepository("courier");
  }
  ...
  async read(params) {
    try {
      ...
      if (search) {
        where[Op.or] = [
          { name: { [Op.like]: `%${search}%` } }, // MySQL LIKE (case-insensitive if collation allows)
        ];
      }
...
const CourierService = new Service();
